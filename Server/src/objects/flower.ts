...
export default class Flower extends NetworkObject{
    ...
    /**
     * Creates a new NetworkObject (Flower implementation)
     * @param spawnAttributes The spawn attributes of the object
     */
    constructor(networkObjectManager:NetworkObjectsManager,spawnAttributes:NetworkObjectSpawnAttributes) {
        super(spawnAttributes);
        this._networkObjectManager = networkObjectManager;
    }
    ...
    private tryToSpawnPollen():void{
        for (let i = 0; i < this._spawnedPollens.length; i++) {
            if(this._spawnedPollens[i] == null){
    ...
    private spawnPollenAt(index:number):void{
        ...
        this._spawnedPollens[index] = this._networkObjectManager.spawnObject(NetworkObjectType.POLLEN,spawnPosition,spawnRotation);
    }
}